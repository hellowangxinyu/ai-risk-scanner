...
| 页面 | 说明 |
|---|---|
| 客户管理 | 客户增删改查、搜索分页；**授信客户标记**（授信=高风险监控档）；Excel 模板导入 + 粘贴文本导入，重复跳过、失败行出报告（行号+原因）；同机构下客户名称唯一，默认机构可在系统配置修改 |
| 风险扫描 | 勾选客户（一键勾选到期客户）→ 后台逐客户串行调用大模型 → 实时进度（Token/估算费用）；批次按日存储、永不覆盖；中断/失败批次可删除或"按原客户清单重扫" |
| 风险台账 | **只显示有风险的记录**；三个视图——"最新批次"（默认）、"每客户最新状态"（最近批次扫干净的客户自动消失）、"**风险项生命周期**"（跨批次去重：每条风险一行，带首次发现/最近确认/出现批次数/活跃或已消失状态，扫描时 AI 会沿用上次的风险标题以保证可关联）；按等级/类型/客户筛选；**三种导出：按扫描日期全量导出、全部历史导出、当前筛选导出**（含授信标识、客户类型等分析列） |
...
1. **到期判定**：从未扫描 → 到期；**客户风险等级由授信状态唯一决定（授信=高、非授信=低），修改授信状态会同步更新等级**；到期 = `上次扫描时间 + 对应档位间隔 ≤ 今天`（高风险 3 天/低风险 30 天，可配）。
2. **快照式存储与生命周期追踪**：每次扫描生成新批次，风险记录按批次隔离、永不覆盖；同一风险跨批次自动关联（客户+类型+标题归一化匹配，AI 扫描时沿用上次标题保证稳定），生命周期视图给出首次发现/最近确认/是否已消失；进程重启时把遗留"运行中"批次标记为"中断"，可重扫或删除。

...
| 系统配置 | AI 接口（Base URL / API Key / 模型名，默认 DeepSeek，任意 OpenAI 兼容服务可换）、博查联网搜索开关、分级扫描周期、每日自动扫描、成本单价、累计用量 |

## 风险数据来源与免责

- 默认数据源：**博查联网搜索（可选）+ DeepSeek 归纳**。搜索失败自动降级为纯模型知识，并在"来源"注明。
...
| API Key | 空 | DeepSeek 平台申请 |
| 模型名称 | `deepseek-chat` | 可改 deepseek-reasoner 等 |
| 博查 API Key | 空 | open.bochaai.com 申请；不填则纯模型知识扫描 |
...
|---|---|---|
| 纯模型知识 | ~¥0.8 | 每天扫 ~¥25/月 |
| 博查搜索 RAG | ~¥6 | 分级周期(3/7/30天) ~¥20–30/月 |
